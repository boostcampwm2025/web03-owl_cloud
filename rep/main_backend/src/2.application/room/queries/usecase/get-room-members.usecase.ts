...
export class GetRoomMembersUsecase<T> {
  ...
  constructor({ selectRoomMemberInfosFromCache }: GetRoomMembersUsecaseProps<T>) {
  ...
  async execute(dto: GetRoomMembersDto): Promise<GetRoomMembersResult> {
    ...
    const result: GetRoomMembersResult = await this.selectRoomMemberInfosFromCache.select({
