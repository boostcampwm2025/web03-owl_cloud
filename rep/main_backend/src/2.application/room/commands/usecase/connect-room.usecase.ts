...
import { NotAcceptRoomMembers, NotAcceptRoomPassword, NotAllowRoomDataUpdate, NotAllowRoomParticipantData, NotInputRoomPassword, NotRoomData, NotRoomInfoData } from "@error/application/room/room.error";
...
export class ConnectRoomUsecase<T, CT> {
  ...
  private readonly compareRoomPasswordHash : ConnectRoomUsecaseProps<T, CT>["compareRoomPasswordHash"];
  private readonly selectRoomInfoDataFromCache : ConnectRoomUsecaseProps<T, CT>["selectRoomInfoDataFromCache"];
  private readonly insertRoomParticipantInfoDataToDb : ConnectRoomUsecaseProps<T, CT>["insertRoomParticipantInfoDataToDb"];
  ...
  async execute(dto : ConnectRoomDto) : Promise<ConnectResult> {
    ...
    const room : RoomProps | undefined = await this.selectRoomDataFromDb.select({ attributeName : "", attributeValue : dto.code }) // 열의 이름은 해당 infra에서 추가하는게 좋아보인다.
    ...
    if ( room.password_hash ) {
      ...
      const roomPasswordChecked : boolean = await this.compareRoomPasswordHash.compare({ value : dto.password, hash : room.password_hash });
      if ( !roomPasswordChecked ) throw new NotAcceptRoomPassword();
    };

    // 2. redis에서 해당 데이터를 찾는다. ( 없으면 에러 )
    const roomInfo : RoomInfoValues | undefined = await this.selectRoomInfoDataFromCache.select({ namespace : room.room_id, keyName : "" }); // room_id만 있으면 된다.
    ...
    try {
      ...
      const insertCacheDataChecked : boolean = await this.insertRoomDatasToCache.insert(insertCacheData);
