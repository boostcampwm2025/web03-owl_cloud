...
export class DisconnectUserUsecase<T> {
  ...
  async execute( user_id : string ) : Promise<void> {
    ...
    const transportInfo : DisconnectUserTransportInfos | undefined = await this.selectUserTransportFromCache.select({ namespace : user_id, keyName : "" });

    // 2. transport_id에 해당하는 값을 찾고 그 값에 transport를 가져오고 close
    const targets : Array<string> = [transportInfo?.send_transport_id, transportInfo?.recv_transport_id].filter(
    ...
    for (const transport_id of targets) {
      ...
      try {
        ...
        try { this.transportRepo.delete(transport_id); } catch {}
      }
    }
