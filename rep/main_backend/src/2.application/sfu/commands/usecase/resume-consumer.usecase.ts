...
export class ResumeConsumerUsecase<T> {
  ...
  async execute(dto : ResumeConsumerDto) : Promise<void> {
    ...
    const checked : boolean = await this.selectConsumerInfoFromCache.select({ namespace : `${dto.room_id}:${dto.user_id}`, keyName : dto.consumer_id });
