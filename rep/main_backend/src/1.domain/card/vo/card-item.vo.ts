...
// x, y값에 대한 타입 
export function cardItemPositionVo( name : string, position : CardItemProps["x"] | CardItemProps["y"] ) : number {
...
// width, height에 대한 타입
export function cardItemSizeVo( name : string, size : CardItemProps["width"] | Exclude<CardItemProps["height"], undefined> ) : number {
...
// rotation에 대한 타입 
export function cardItemRotationVo( rotation : CardItemProps["rotation"] ) : number {
...
// scale에 대한 타입 
export function cardItemScaleVo( name : string, scale : CardItemProps["scale_x"] | CardItemProps["scale_y"] ) : number {
...
// opacity에 대한 타입 
export function cardItemOpacityVo( opacity : Exclude<CardItemProps["opacity"], undefined> ) : number {
  ...
  const min : number = 0;
  const max : number = 1;
  if ( opacity < min || opacity > max ) throw new NotAllowRangeType({ name, max, min });

  return opacity;
};

// z_index에 대한 타입
export function cardItemZindexVo( z_index : Exclude<CardItemProps["z_index"], undefined> ) : number {
...
// is_locked에 대한 타입 체크
export function cardItemIsLockedVo( is_locked : Exclude<CardItemProps["is_locked"], undefined> ) : boolean {
...
export function cardItemIsVisibleVo( is_visible : Exclude<CardItemProps["is_visible"], undefined> ) : boolean {
...
// name에 대한 타입 체크
export function cardItemNameVo( name : Exclude<CardItemProps["name"], undefined> ) : string {
  ...
  if ( name.length > length ) throw new NotAllowMaxLengthText({ name : title, length });
